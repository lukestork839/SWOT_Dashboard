...
```
ANCHOR_LAT = 59.82463509  # North
ANCHOR_LON = -161.33397834  # West
```
...
**Updated (2026-05-20):** Ice season narrowed to Dec-Mar based on data analysis. Dashboard now **filters ice months by default** with opt-out checkbox.
...
- Smooth ice → classified as water (Class 3-4) and **passes through quality filters**
...
**Data-validated ice seasons for Kanektok/Uyak (~59.8°N) (2026-05-20):**
Analysis of 170 satellite passes (Jul 2023 – May 2026) revealed:
- **Oct-Nov**: Ice-free in data (C4 ratio 30-40%, same as summer) — contradicts earlier "Oct-May" assumption
- **Dec-Mar**: Peak ice contamination — Uyak Creek 80-95% Class 4 (vs 35-55% open water), Kanektok 58-77%
- **Apr-May**: Transitional but mostly usable — active breakup, upstream lags downstream
- **Jun-Sep**: Fully ice-free (confirmed across all 3 freeze-thaw cycles observed)
- **Uyak freezes more completely** than Kanektok (narrow/shallow channel → smooth ice → high Class 4)
- **Upstream thaws later** than downstream (May 2026: only 206 Uyak pixels upstream vs 10k downstream)

**Current approach (2026-05-20):**
- **Ingestion** (`SWOT_Pull.py`): All data preserved — no date-based filtering at download/processing level
- **Dashboard** (`dashboard_swot.py`): Ice months **excluded by default** via sidebar checkbox
  - "Exclude ice season (Dec-Mar)" checkbox, checked by default
  - When checked: `WHERE ... AND MONTH(Pass_Date) NOT IN (12, 1, 2, 3)` added to SQL
  - When unchecked: `get_ice_warning()` fires, shows `st.warning()` banner explaining smooth ice contamination
  - Seasonal Comparison and Typhoon Impact tabs unaffected (use their own date ranges via `query_period_data()`)
...
### Directory Structure (Updated 2026-05-13)
```
SWOT/
├── SWOT_Pull.py                    # Main data ingestion + optimization pipeline
├── dashboard_swot.py          # Streamlit visualization dashboard
├── requirements.txt            # Dashboard-only dependencies (used by Streamlit Cloud)
├── requirements-full.txt       # Full dependencies (ingestion + dashboard, local dev)
├── river_poly.zip              # Polygon boundaries (2 reaches)
├── .swot_cli_config.json       # SWOT CLI configuration
├── batch_outputs/              # Processed data directory (local only, gitignored)
...
3. **Map Display Options** (Updated 2026-05-26):
   ...
   - **Color by Detrended Residual**: Diverging RdBu colormap, 95th percentile scale (prevents outlier skew)
   - **Color by Interval Slope**: Sequential YlOrRd colormap
   - **Basemap Selection**: OpenStreetMap, Satellite (ESRI, default)
   - **Controls**: Inline in Map View tab (not sidebar), wrapped in `@st.fragment` for isolated reruns
   - **Rendering**: `folium.GeoJson` with vectorized colors (replaces slow `iterrows` + `CircleMarker`)
   - **Legend**: Custom `VerticalColorbar` class (Leaflet L.control, positioned top-left below zoom/ruler)
   - **Measuring Tool**: Click-to-measure distances and areas on map
   - **Layer Control**: Toggle river/classification layers on/off
4. **Detrending Method**: Hardcoded to Polynomial (2nd order) — selector removed from sidebar (2026-05-26)
5. **Tabs**:
...
**Data Loading (Updated 2026-05-13):**
Two-tier data source priority:
1. **Local**: `batch_outputs/master_all_data_part_*.parquet` (from SWOT_Pull.py)
2. **Remote**: DuckDB `httpfs` reads parquet directly from GitHub Releases URL (Streamlit Cloud)
...
# Remote mode (Streamlit Cloud) — DuckDB fetches only needed data chunks via HTTP range requests
...
**Dashboard Tab Order (Updated 2026-04-06):**
**Top-level tabs:**
1. Gradient Profile (original)
2. Detrended Profile (added 2026-02-16)
3. Map View (enhanced with Folium 2026-02-13)
4. More Tabs — contains less-used tabs as nested sub-tabs:
   - Elevation Difference (added 2026-02-13)
   - Slope Profile (redesigned 2026-04-08, was "Interval Slopes")
   - Raw Data (original)
   - Temporal Evolution (added 2026-02-23)
   - Seasonal Comparison (added 2026-04-01)
   - Typhoon Impact (added 2026-04-01)
...
### 2026-04-08: Light Theme & Slope Profile Redesign
**Problems Addressed:**
- Light mode toggle only changed Streamlit background, not Plotly chart backgrounds (charts stayed dark)
- Interval Slopes tab was extremely noisy (500-700 cm/km spikes) due to 100m bin-to-bin finite differences on raw SWOT data

**Actions Taken:**
1. ✅ **Light theme fix**: Changed `.streamlit/config.toml` from `base = "dark"` to `base = "light"` and updated default Plotly template to `plotly_white`. Added `theme=None` to all `st.plotly_chart` calls to prevent Streamlit theme override.
2. ✅ **Slope Profile redesign**: Replaced noisy bin-to-bin slope calculation with derivative of fitted curve:
   - New `calculate_slope_profile()` function fits a 2nd order polynomial per river using `np.polynomial.Polynomial.fit()`
   - Computes analytical derivative via `.deriv()` — no numerical noise
   - Evaluates at 200 evenly-spaced points along each river
   - Produces smooth, poster-ready slope curves (~15-40 cm/km range)
3. ✅ Renamed tab from "Interval Slopes" to "Slope Profile"

**Technical Details:**
- Polynomial derivative: `poly.deriv()` returns exact analytical derivative (1st order poly from 2nd order fit)
- Slope in cm/km: `deriv_poly(x_eval) * 100` (converts m/km to cm/km)
- Old noisy SQL with LAG/LEAD window functions replaced by simple data query + Python fitting
- Map view "Interval Slope" color-by option still uses pre-computed bin slopes (unchanged)

**Additional dashboard changes for poster (2026-04-08):**
- Switched Streamlit config to `base = "light"` permanently (`.streamlit/config.toml`)
- Default Plotly template now `plotly_white` in both light_mode branches
- Added `theme=None` to all 15 `st.plotly_chart` calls to prevent Streamlit theme override
- X-axis label changed from "Distance from Confluence Anchor (km)" to "Distance from Anchor Point (km)" across all plots (anchor moved slightly past confluence)
- Gradient Profile refactored from `px.scatter` to `go.Scatter` traces with solid legend markers (invisible dummy trace with `x=[None]` for legend, actual data trace with `showlegend=False`)
- Same solid-legend-marker pattern applied to Detrended Profile
- Slope Profile `calculate_slope_profile()` revised: replaced polynomial derivative approach with Gaussian-smoothed binned medians (100m bins, 2km smoothing window, `np.gradient` for numerical derivative) — produces physically realistic slope variation instead of linear ramp from polynomial
- `plotly.io` (`pio`) imported for template management
- Poster assets created in `poster_assets/` directory: `method_comparison.svg`, `conclusions.svg`, `poster_layout.svg`, `swot_figure_grid.svg`

**Poster completed (2026-04-08):**
- Title: "Mapping Avulsion Risk of the Kanektok River"
- Dimensions: 48×36 inches (4×3 ft), built in PowerPoint
- Key finding: Slopes 181 vs 180 cm/km, avulsion risk unlikely
- Sections: Study area, SWOT WSE analysis (4 figures), Preliminary ArcticDEM analysis, Method comparison, Conclusions, Problem statement, Next steps, References
...
```bash
pip install -r requirements-full.txt   # Full deps (ingestion + dashboard)
pip install -r requirements.txt        # Dashboard-only deps
```
...
# Uses local data from batch_outputs/ if available
# Falls back to remote parquet from GitHub Releases if no local data
```
...
- ✅ **Streamlit Cloud overhaul** (2026-05-13): Replaced download-based approach with DuckDB httpfs remote parquet reading, trimmed requirements.txt to dashboard-only deps, removed sample_data from repo, deployed successfully at swotdashboard.streamlit.app
...
- ✅ **Ice season filtering** (2026-05-20): Data-validated ice months (Dec-Mar, not Oct-May), dashboard excludes by default with opt-out checkbox and warning banner
- ✅ **Dashboard simplification** (2026-05-26): Removed detrending/theme selectors, trimmed basemaps, removed WSE color option, hardcoded Poly 2nd order and light mode
- ✅ **Map performance overhaul** (2026-05-26): `@st.fragment` for isolated map reruns, `folium.GeoJson` replaces slow `iterrows`+`CircleMarker`, vectorized color computation, custom `VerticalColorbar` legend, 95th percentile residual scale
...
### 2026-04-06: Anchor Point Update, Tab Reorganization & Light Mode
**Actions Taken:**

1. ✅ **Moved anchor point upriver** — River polygons extended past bifurcation point, causing X-axis issues. New anchor: `59.82463509°N, -161.33397834°W` (shifted east/upriver from old `59.826973°N, -161.372337°W`). Requires full data reprocessing since all distances are recalculated.

2. ✅ **Reorganized dashboard tabs** — Pocketed rarely-used tabs into a nested "More Tabs" tab. Top-level tabs now: Gradient Profile, Detrended Profile, Map View, More Tabs (containing Elevation Difference, Interval Slopes, Raw Data, Temporal Evolution, Seasonal Comparison, Typhoon Impact).

3. ✅ **Added light/dark theme toggle** — Sidebar toggle "Light mode (for screenshots/posters)" switches all Plotly charts from `plotly_dark` to `plotly_white` and injects CSS to turn Streamlit background/text to white/black. Default remains dark mode. Purpose: poster-ready screenshots on white background.

### 2026-05-11: Dashboard Bug Fixes & Typhoon Tab Overhaul
**Actions Taken:**

1. ✅ **Fixed Elevation Difference max metric** — `diff_df['elevation_diff'].max()` only found the most positive value (Kanektok above Uyak), ignoring large negative values where Uyak is higher. Replaced single "Max Difference" with three metrics:
   - **Max |Difference|**: Largest absolute magnitude regardless of sign
   - **Kanektok Max Above**: Most positive value
   - **Uyak Max Above**: Most negative value (where Uyak overtops Kanektok)

2. ✅ **Typhoon Impact tab: ice contamination fix** — The "Immediate Impact" section compared open-water (Aug-Sep) to freeze-up (Oct-Dec), introducing a 0.5-2+ m ice artifact in WSE. Restructured the entire tab:
   - **Promoted same-season comparison** to primary analysis (Section A). Compares Summer 2025 vs Summer 2026 open-water data when available.
   - **Added interim same-month year-over-year comparison** — Automatically finds the latest month available in both 2025 and 2026 (currently March). Both panels share the same ice conditions so artifacts largely cancel. Warning directs users to focus on **slope changes, not absolute WSE**.
   - **Demoted immediate before/after** into a collapsed `st.expander` with a prominent `st.error` warning explaining the ice contamination. Kept for reference but clearly labeled as unreliable.
   - **Binned elevation change** now uses same-season data instead of ice-contaminated cross-season data.
   - **Methodology explanation** added at top of tab explaining why same-season comparison is necessary.
   - Interim comparison auto-replaces once summer 2026 data is available.

**Data availability note (2026-05-11):** Dataset covers Jul 2023 – Mar 2026. No post-typhoon open-water data yet. Interim comparison uses Mar 2025 vs Mar 2026 (6 passes each, good coverage). Summer 2026 open-water comparison will be available after re-running `SWOT_Pull.py` in June-July 2026.

### 2026-05-13: Streamlit Cloud Deployment — Remote Parquet via DuckDB httpfs
**Problem Addressed:**
- Online dashboard at swotdashboard.streamlit.app was timing out / not loading
- Previous approach had three issues: (1) bloated requirements.txt with heavy ingestion-only packages (earthaccess, xarray, geopandas, netCDF4, shapely, pyproj) slowing install and eating memory, (2) bundled sample_data in git repo, (3) fallback download of 24MB parquet from GitHub Release on every cold start

**Solution: DuckDB httpfs**
Instead of downloading or bundling data, the dashboard now uses DuckDB's `httpfs` extension to read parquet files directly from a GitHub Release URL. DuckDB uses HTTP range requests to fetch only the data chunks needed per query — no full download required.

**Actions Taken:**
1. ✅ **Split requirements.txt** — `requirements.txt` now contains dashboard-only deps (streamlit, plotly, duckdb, folium, etc.). Created `requirements-full.txt` with all deps including ingestion pipeline packages. Streamlit Cloud auto-detects `requirements.txt`, so it installs only what it needs.
2. ✅ **Generated fresh parquet** — Full-resolution May-Jul 2025 data (685,612 rows, 15 passes, both rivers, 22.5MB) with Pass_Date stored as proper DATE type (not VARCHAR).
3. ✅ **Uploaded to GitHub Release v2.0-data** — Replaced old v1.0-data release. Download URL: `https://github.com/lukestork839/SWOT_Dashboard/releases/download/v2.0-data/swot_may_jul_2025.parquet`
4. ✅ **Rewrote get_database_connection()** — Two-tier priority: local partition files first, remote httpfs second. Removed `download_data_if_needed()` function entirely.
5. ✅ **Removed sample_data/** — No longer needed; remote parquet replaces bundled data.
6. ✅ **Updated README** — Data loading priority, project structure, configuration reference, troubleshooting all updated.

**Bug Fix: DuckDB httpfs VARCHAR truncation**
- `MIN()`/`MAX()` on VARCHAR columns over httpfs returned truncated strings (e.g., `'2025-05-'` instead of `'2025-05-08'`)
- Root cause: DuckDB httpfs optimization reads minimal bytes for VARCHAR aggregation
- Fix: Store Pass_Date as DATE type in the parquet file — MIN/MAX returns proper datetime objects

**Performance (remote parquet queries):**
| Query | Time |
|-------|------|
| Row count | ~1.2s |
| Date range metadata | ~1.0s |
| Stats (AVG with filter) | ~2.1s |
| Filtered count | ~0.5s |

**Deployment:**
- Dashboard live at: https://swotdashboard.streamlit.app/
- Auto-deploys on push to main branch
- Free tier: app sleeps after inactivity, wakes in ~30s on visit
- No server or database to maintain — Streamlit Cloud + GitHub Releases only

**Files Modified:**
- `dashboard_swot.py`: Rewrote data loading, removed download function, added REMOTE_PARQUET_URL config
- `requirements.txt`: Trimmed to dashboard-only deps
- `requirements-full.txt`: New file, full deps for local ingestion
- `README.md`: Updated data loading, structure, config, troubleshooting
- `sample_data/sample_data.parquet`: Deleted

**Key Decision: Why not bundle data in git?**
- Git repos aren't designed for large binary files — bloats clone size permanently
- GitHub Releases can host up to 2GB per file, free, version-controlled
- DuckDB httpfs fetches only needed chunks, so even large remote files are queryable efficiently
- To update online data, just upload a new parquet to a new release — no code changes needed

### 2026-05-18: Data Update & Ice Season Filtering
**Actions Taken:**

1. ✅ **Data update through May 2026**
   - Queried NASA CMR API to determine latest available SWOT data globally (May 14, 2026)
   - Ran `SWOT_Pull.py` for 2026-01-01 to 2026-05-18 — 54 granules found, 8 new passes downloaded, 40 skipped (resumable)
   - New passes: Apr 7, Apr 8, Apr 17, Apr 18, Apr 28, Apr 29, May 8, May 9
   - Master files rebuilt from 170 daily CSVs (8.4M total points, 85 partitions)
   - SWOT orbit: ~21-day repeat cycle, passes over study area every ~10-11 days in pairs
   - AVISO pass locator identified as best resource for predicting future overpasses

2. ✅ **Ice contamination analysis**
   - Analyzed Class 4 ratios and point counts across all 170 passes by month
   - **Finding**: Smooth river ice passes through Class 3-4 filter during Dec-Mar
     - Uyak Creek: 80-95% Class 4 during peak freeze (vs 35-55% open water)
     - Kanektok River: 58-77% Class 4 (wider river, less complete freeze)
   - **Finding**: Oct-Nov are ice-free (contradicts earlier "Oct through May" assumption)
   - **Finding**: Uyak upstream thaws later — May 9 had only 206 pixels upstream vs 10k total
   - **Finding**: Early April data (124k Uyak points) is likely ice misclassified as water

3. ✅ **Dashboard ice season filtering**
   - Updated `ICE_AFFECTED_MONTHS` from `{10,11,12,1,2,3,4,5}` to `{12,1,2,3}` (data-validated)
   - Updated `OPEN_WATER_MONTHS` from `{6,7,8,9}` to `{4,5,6,7,8,9,10,11}`
   - Simplified `ICE_SEASONS` dict — removed `freeze_up` and `break_up`, kept only `frozen` (Dec-Mar)
   - Added "Exclude ice season (Dec-Mar)" checkbox in sidebar form (checked by default)
   - When checked: appends `AND MONTH(Pass_Date) NOT IN (12,1,2,3)` to WHERE clause
   - When unchecked: activates `get_ice_warning()` to show warning banner
   - Updated README.md ice awareness section with data-backed boundaries

**Key References:**
- NASA CMR API: `https://cmr.earthdata.nasa.gov/search/granules.json?short_name=SWOT_L2_HR_PIXC_D`
- Earthdata Search: `https://search.earthdata.nasa.gov/search/granules?p=C3233944986-POCLOUD`
- AVISO Pass Locator: `https://www.aviso.altimetry.fr/en/data/tools/pass-locator/swot-scientific-orbit.html`
...
1. ✅ Removed detrending method selector — hardcoded to Polynomial (2nd order), method name shown in graph axis labels
2. ✅ Removed Display Theme toggle — hardcoded to light mode (`plotly_white`)
3. ✅ Trimmed basemap options to OpenStreetMap and Satellite (ESRI), default to Satellite
4. ✅ Moved map display controls (Color By, Basemap, Opacity) from sidebar into Map View tab as inline 3-column row
5. ✅ Removed WSE color-by option (redundant — elevation trend visible in Gradient Profile)

**Map Performance:**
6. ✅ Wrapped map tab in `@st.fragment` — changing map controls only re-renders the map, not the entire dashboard
7. ✅ Replaced `for _, row in df.iterrows()` + individual `folium.CircleMarker` with `folium.GeoJson` — Leaflet renders all 5,000 points in a single `L.geoJson` call on the client side
8. ✅ Vectorized color computation — numpy computes all hex colors in one shot for continuous colormaps
9. ✅ GeoJSON features built via `zip()` list comprehension (fast) instead of `iterrows()` (slow)

**Legend Fix:**
10. ✅ Replaced `branca.colormap.LinearColormap` (horizontal, clipped at map edge) with custom `VerticalColorbar` class
    - Uses Leaflet `L.control({position: 'topleft'})` — stacks below zoom and ruler controls
    - Vertical gradient bar with min/mid/max labels
    - Built with `branca.element.MacroElement` + Jinja2 template

**Residual Color Scale Fix:**
11. ✅ Detrended Residual color scale now uses 95th percentile of `abs(residual)` with `clip=True` instead of `abs().max()`
    - Prevents outliers (e.g., ±23m) from washing out the entire color range
    - Outliers beyond the bound still render as the most extreme color (deep red/blue)
    - Floor of 0.5m minimum to prevent near-zero scale

**Imports Changed:**
- Removed: `branca.colormap.LinearColormap` (and try/except fallback)
- Added: `branca.element.MacroElement`, `jinja2.Template as JinjaTemplate`

### In Progress
- ⏳ **Quality flag filter tuning** — Awaiting SWOT expert guidance on which bit flags to exclude for narrow rivers
- ⏳ **Summer 2026 data** — Needed for definitive open-water typhoon impact comparison (available Jun-Aug 2026)
- ✅ **Expanded online dashboard data** (2026-05-27) — Now serves Apr-Jul 2025 + Apr-May 2026 (1.38M rows, 29 passes). Uses stable filename `dashboard_data.parquet` for hot-swap updates
- ⏳ **Thesis documentation** — Ice contamination findings (Dec-Mar, smooth ice as Class 4) need to be included in thesis write-up
...
- Upload full dataset (Jul 2023 – present) to GitHub Release for complete online dashboard experience
...
- **Remote Parquet URL**: `dashboard_swot.py` line 33 (`REMOTE_PARQUET_URL`)
- **River Color Mapping**: `dashboard_swot.py` lines 38-42
- **Map Color-by Options**: `dashboard_swot.py` (inline in Map View tab, inside `@st.fragment`)
- **Max Plot Points**: `dashboard_swot.py` line 34
- **Git Ignore Rules**: `.gitignore` (root directory)

### Data Paths
- **Polygon Boundaries**: `river_poly.zip` (root directory, refined 2026-02-11)
- **Output Directory**: `batch_outputs/` (gitignored, local only)
...
- **Dashboard-only deps**: `requirements.txt` (used by Streamlit Cloud)
- **Full deps**: `requirements-full.txt` (ingestion + dashboard, local dev)
...
- **Folium Map View**: `dashboard_swot.py` tab5 — `render_map()` `@st.fragment` function with GeoJson rendering
- **Map Color Modes**: `dashboard_swot.py` (inside `render_map()`, uses `folium.GeoJson` + vectorized colors)
- **VerticalColorbar**: `dashboard_swot.py` class (Leaflet L.control, MacroElement + Jinja2 template)
- **Map Controls**: `dashboard_swot.py` (inline in map tab, 3-column layout: Color By, Basemap, Opacity)
