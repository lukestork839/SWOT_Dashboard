...
| 33 | DEFER-SPLIT → RESOLVED (split PR B2, 2026-08-16) | np.round (banker's) vs DuckDB ROUND (half-away) binning convention — `swot_core.stats.round_half_away` added and adopted at the 5 numpy binning sites (slope-profile 0.1 km, elevation-difference 0.1 km, gradient-node 0.5 km, thesis Figs 5/7 nodes); the SQL ROUND sites already half-away, unchanged. Re-measured on the archive: 14 exact-boundary points at the 0.1/0.5 km widths (7 change bins) + 10 at 1.0 km = the 24 triaged. Gate: 21 expected diffs, all ≤1e-5 cm/km / ≤0.03 mm — nothing moves at any reported precision. AMENDED (post-review, 2026-08-16): the tie CONVENTION was only half the agreement — the division must also happen in float32 like DuckDB's REAL/DECIMAL; `calculate_slope_profile`'s callers pass `.tolist()` (float64), where 17 archive points lose their exact-tie status. Now casts to float32 before dividing (0 mismatches vs SQL archive-wide, verified). CORRECTION: "10 at 1.0 km (SQL-only)" was wrong — `SWOT_Pull.py:516` (reference-gradient nodes) and `:166` (MAD node reference) are numpy 1.0 km binning sites still on banker's; conversion deferred to a follow-up PR because it touches the canonical refgrad artifact (measured: 2 of 190 per-pass Theil–Sen values shift ≤0.008 cm/km, medians unchanged) |
| 42 | DEFER-SPLIT → RESOLVED (split PR B2, 2026-08-16) | Two coexisting 2nd-order detrend fits (map vs tab) — the Map View residuals now evaluate the canonical `load_detrend_frame` fit (the Detrended Profile tab's) at the plot sample's distances instead of refitting on the sample. Measured disagreement removed: up to 2.97 cm on the 2026 selection (≤9 cm worst-case per the original triage). AMENDED (post-review, 2026-08-16): `load_detrend_frame` now returns the fit's coefficients directly — the first cut re-passed the ~31k-element frame columns through a second cached call, whose cache-KEY hashing (~0.3 s) cost ~50× the fit it avoided; and `calculate_detrending` now returns ascending polyval-ready coeffs for EVERY method (Linear was `[slope, intercept]`, a latent garbage-residuals hazard if the hardcoded 2nd-order method ever changed) |
