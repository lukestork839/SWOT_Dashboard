...
| E6 | **Arc β = (crest − floodplain)/(crest − bed)**, Kanektok, NaN unless H_M>0 | `build_arc_B.py:319-325` | ✅ **resolved 2026-08-19**: the preliminary-ArcGIS comparison was removed from the analysis entirely, so the "identical to (P98−med)/(P98−P2)" claim is gone from every doc. β now stands on its own as the Gearon superelevation ratio answering CIVIC warning sign 1 (β > ~0.5, Ganti et al. 2016), with the threshold stated as one-directional | OK (Gearon 2024; Ganti et al. 2016 for the threshold) |
| E7 | ~~ArcGIS-reproduction β = (P98−median)/(P98−P2)~~ | *removed* | ➖ **retired 2026-08-19** — the preliminary-ArcGIS β reproduction is no longer part of this analysis; `beta_floodplain.py`, `reproduce_beta.py`, `prototype_B.py`, `run_B.py`, `make_beta_figures.py`, `recover_original_beta.py` and `reference/original_beta.parquet` were deleted (recoverable from git history). Those preliminary results live outside this repo | n/a |
